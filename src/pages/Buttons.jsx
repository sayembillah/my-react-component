      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {buttonData.map((button, index) => (
          ...
          >
            {/* Copy Button */}
            <button
              onClick={() => handleCopy(button.desc)}
            ...
            <div className="text-lg font-semibold">{button.button}</div>
            <div className="text-sm text-gray-300 mt-2">{button.name}</div>
            <div className="text-xs text-gray-400 mt-1">{button.desc}</div>
          </div>
        ))}
      </div>
    </div>
